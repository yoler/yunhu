...
Page({
  ...
  data: {
      ...
      showAd: false,
      tips: true,
      colorList: ['#f2f1f6', '#c3d3ea', '#e2edf3', '#ece8e5', '#f1e8e3', '#d2e8ef', '#fff0d3', '#e3d4cd', '#e7cfb7', '#f6d8b6']
  },
  ...
  getPhoto () {
    wx.request({
      ...
      success: (result) => {
        this.setData({
          list: [...this.data.list, {data: result.data.data, color: this.data.colorList[Math.floor(Math.random()*(this.data.colorList.length))]}]
        })
        wx.hideLoading()
      },
    })
